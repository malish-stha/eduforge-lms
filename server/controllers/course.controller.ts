...
import { complexSearch } from "../utils/searchAlgorithm";
...
export const searchCourses = async (
  ...
  try {
    ...
    const courses = await CourseModel.find().select("name"); // Assuming CourseModel is already imported

    if (typeof query === "string") {
      const courseNames = courses.map((course) => course.name);
      const results = complexSearch(courseNames, query);
      res.json(results);
    } else {
